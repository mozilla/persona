...
rm -rf rpmbuild/RPMS rpmbuild/SOURCES/persona

tar --exclude rpmbuild --exclude .git --exclude .svn \
    --exclude var -czf \
    "$PWD/rpmbuild/SOURCES/persona-server.tar.gz" .
...
rpmbuild --define "_topdir $PWD/rpmbuild" \
         --define "svnrev $SVN_REVISION" -ba scripts/persona.spec
rc=$?
if [ $rc -eq 0 ]; then
    ls -l $PWD/rpmbuild/RPMS/*/*.rpm
else
    echo "$progname: failed to build persona RPM (rpmbuild rc=$rc)" >&2
fi
