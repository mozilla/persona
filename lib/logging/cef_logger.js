 ...
 *
 * All cef messages are emitted to the Winston logger, where they are
 * picked up by winston-cef and written to the cef logger.
 */
...
Logger.prototype = {
  ...
  _emit: function(severity, signature, name, extensions) {
    ...
    logger[severity]('cef.' + name, options);
  },
  ...
  emergency: function(signature, name) {
    ...
    return this._emit('error', signature, name, extensions);
  },

  alert: function(signature, name) {
    var extensions = mergeObjects([].slice.call(arguments, 2));
    return this._emit('help', signature, name, extensions);
  },
