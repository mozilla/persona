...
var MetricsFileLogger = function(options) {
  options = _.extend({
...
util.inherits(MetricsFileLogger, File);

MetricsFileLogger.prototype.name = 'metricsFileLogger';

MetricsFileLogger.prototype.log = function(level, msg, meta, callback) {
  if (!/^metrics\./.test(msg)) return callback(null, true);

  msg = msg.replace(/^metrics\./, '');
  return File.prototype.log.call(this, level, msg, meta, callback);
};
