...
var fetchWellKnown = function (currentDomain, principalDomain, clientCB) {
  ...
  var cb = function(err) {
    ...
    if (err)
      logger.info('elapsed_time.fetch_well_known.error', reqTime);
    else
      logger.info('elapsed_time.fetch_well_known.success', reqTime);
...
exports.checkSupport = function(principalDomain, cb) {
  ...
  deepFetchWellKnown(principalDomain, function (err, r) {
    ...
    if (err) {
      logger.info('elapsed_time.deep_fetch_well_known.error', reqTime);
      logger.debug(err);
      cb(err);
    } else if (r) {
      logger.info('elapsed_time.deep_fetch_well_known.success', reqTime);
      if (r.disabled) {
