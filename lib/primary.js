...
const IDP_GRACE_PERIOD = config.get('idp_offline_grace_period_ms');
...
exports.currentSupport = function currentSupport(domain) {

  return Q.all([

    Q.nfcall(exports.checkSupport, domain).fail(function(err) {
      // error isn't fatal, we'll just log it and carry on
      logger.info('"' + domain + '" primary support is misconfigured, falling back to secondary: ' + err);
    }),

    Q.nfcall(db.getIDPLastSeen, domain)

  ]).spread(function(support, when) {
    var supportInfo;
    if (support && support.urls){
      supportInfo = support.urls;
      supportInfo.type = 'primary';
      supportInfo.issuer = support.authoritativeDomain;
    } else {
      supportInfo = {
        type: 'secondary',
        issuer: HOSTNAME,
        disabled: (support && support.disabled) || false
      };
    }

    if (when && supportInfo.type === 'secondary') {
      // if we've seen it recently, and it's not disabled, then this
      // isn't a secondary address. we must treat it like an offline
      // primary.
      if (((Date.now() - when) < IDP_GRACE_PERIOD) && !supportInfo.disabled) {
        supportInfo.offline = true;
        supportInfo.type = 'primary';
      }
      supportInfo.seen = true;
    }
    return supportInfo;
  });
};
