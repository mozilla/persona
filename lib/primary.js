...
var fetchWellKnown = function (currentDomain, principalDomain, cb) {
...
function deepFetchWellKnown(principalDomain, cb, currentDomain, delegationChain) {
  ...
  fetchWellKnown(currentDomain, principalDomain, function(err, unparsedDoc) {
    ...
    if (supportDoc.type === 'disable')
    {
      return cb(currentDomain + ' has explicitly disabled browserid support');
    }
...
exports.delegatesAuthority = function (emailDomain, issuingDomain, cb) {
  exports.checkSupport(emailDomain, function(err, r) {
    if (err || !r) return cb(false);
    // There are two conditions under which delgation is allowed:
    // 1. (normal delegation) the emailDomain explicitly delegates to the issuingDomain
    // 2. (proxy idp) we run a proxy for the emailDomain (who themselves has no support),
    //    and the issuer is us.
    if (r.authoritativeDomain === issuingDomain) {
      // case 1
      cb(true);
    } else if (r.proxied && issuingDomain === HOSTNAME) {
      // case 2
      cb(true);
    } else {
      cb(false);
