...
// This becomes async
function parseWellKnownBody(body, fromDomain, delegates, cb) {
  var v;
  try {
    v = JSON.parse(body);
  } catch(e) {
  ...
  got.forEach(function (k) {
    if ('authority' === k) {
      ...
      getWellKnown(v[k], delegates, function (err, nbody, newDomain, ndelegates) {
        if (err) {
          bail = true;
          return cb(err);
        }
        parseWellKnownBody(nbody, newDomain, ndelegates, cb);
      });
      bail = true;
    }
  });
...
var getWellKnown = function (fromDomain, delegates, cb) {
...
exports.checkSupport = function(principalDomain, cb, delegates) {
  ...
  getWellKnown(principalDomain, delegates,
    ...
    if (err) {
      ...
      try {
        parseWellKnownBody(body, newDomain, cbdelegates, function (err, r) {
          if (err) {
            logger.debug(err);
            cb(err);
          } else {
