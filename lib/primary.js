...
// this will call cb(err) or cb(null, URLs, publicKey, authorityDomain)
      ...
      try {
        var r = parseWellKnownBody(body, domain, cbdelegates, function (err, r) {
          if (err) {
            ...
            return cb(null, r.urls, r.publicKey, r.authorityDomain);
          }
...
exports.getPublicKey = function(domain, cb) {
  exports.checkSupport(domain, function(err, urls, publicKey, authorityDomain) {
    if (publicKey === null) {
...
exports.delegatesAuthority = function(emailDomain, issuingDomain, acceptSecondary, cb) {
  exports.checkSupport(emailDomain, function(err, urls, publicKey, authorityDomain) {
    if (err)
      return cb(false);
    // * If emailDomain is a primary, authorityDomain==emailDomain
    // * If they delegate, authorityDomain is their delegate
    // * proxyIDPs are treated as delegates
    if (authorityDomain) {
      if (g_shim_cache.hasOwnProperty(authorityDomain)) {
        authorityDomain = g_shim_cache[authorityDomain].origin;
        authorityDomain = authorityDomain.replace(/^.*\/\//, "");
        authorityDomain = authorityDomain.replace(/[:\/].*$/, "");
      }
      if (issuingDomain === authorityDomain)
        return cb(true);
      return cb(false);
    }
    // if they are not a primary, authorityDomain is null
    if (acceptSecondary && (issuingDomain === HOSTNAME))
      return cb(true); // accept the secondary
    return cb(false);
  });
}
...
exports.verifyAssertion = function(assertion, cb) {
  ...
  jwcrypto.cert.verifyBundle(assertion, now, getRoot, function(err, certParamsArray, payload, assertionParams) {
    ...
    // This does not accept assertions backed by the browserid
    // fallback/secondary. It only accepts assertions backed by:
    // * a primary
    // * a key to which the primary has delegated authority
    // * a key from an authority that proxyIDPs[domain] points to

    // Our delegatesAuthority() function will tell us whether to accept the
    // issuer. It isn't ideal (it doubles the latency and HTTPS calls from
    // our earlier call to getPublicKey), but provides us with the right
    // information.
    ...
    exports.delegatesAuthority(domainFromEmail, rootIssuer, false,
                               function(delegated) {
      if (!delegated)
