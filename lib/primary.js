...
function parseWellKnownBody(body, domain, delegates, cb) {
  ...
  try {
    ...
    return process.nextTick(function() {
      if (cb) cb("malformed declaration of support for '" + domain + "': " + String(e));
    });
  }
...
var getWellKnown = function (domain, delegates, cb) {
  ...
  var reqTimeout = setTimeout(function() {
    req.abort();
    logger.debug('timeout trying to load well-known for ' + domain);
    handleProxyIDP();
  }, primaryTimeout);
  ...
  req.on('error', function(e) {
    ...
    logger.debug(domain + ' is not a browserid primary: ' + String(e));
    handleProxyIDP();
  });
};
...
exports.checkSupport = function(domain, cb, delegates) {
  ...
  getWellKnown(domain, delegates, function (err, body, domain, cbdelegates) {
    if (err) {
      ...
      try {
        parseWellKnownBody(body, domain, cbdelegates, function (err, r) {
          if (err) {
            logger.debug(err);
            cb(err);
          } else {
        ...
        var msg = domain + ' is a broken browserid primary, malformed dec of support: ' + String(e);
        logger.debug(msg);
        return cb(msg);
      }
    }
