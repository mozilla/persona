...
exports.setup = function(options, app) {
  ...
  fs.readdirSync(path.join(__dirname, 'wsapi')).forEach(function (f) {
    ...
    try {
      ...
      // don't register read apis if we are configured as a writer, except for
      // have_email which we'll register on dbwriter as well (see issue #1258).
      // We explicitly allow have_email on the dbwriter
      // so that monitoring software has a hook to test db connectivity
      // (this will change in dev to an API specifically for this purpose).
      if (options.only_write_apis && !api.writes_db && operation !== 'have_email') return;
