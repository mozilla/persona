...
exports.emailType = function(email, cb) {
  sync();
  var m = jsel.match(".emails ." + ESC(email), db.users);
  process.nextTick(function() { cb(null, m.length ? m[0].lastUsedAs : undefined); });
};
...
exports.emailInfo = function(email, cb) {
  ...
  var response = {
    lastUsedAs: m[0].lastUsedAs || "secondary"
  };
    ...
    exports.emailKnown(o.email, function(err) {
      ...
      exports.emailToUID(o.email, function(err, uid) {
        ...
        Object.keys(emails).forEach(function(email) {
          if (email !== o.email && emails[email].lastUsedAs === 'secondary') {
...
exports.listEmails = function(uid, cb) {
  ...
  process.nextTick(function() {
    ...
    // client expects .type, not .lastUsedAs, so we munge internal
    // representation into external
    var rv = {};
    Object.keys(emails[0]).forEach(function(email) {
      console.log(emails[0]);
      rv[email] = {
        type: emails[0][email].lastUsedAs,
        verified: emails[0][email].verified
      };
    });
