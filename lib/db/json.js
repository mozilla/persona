...
exports.emailInfo = function(email, cb) {
  sync();
  var m = jsel.match(".emails ." + ESC(email), db.users);
  if (!m.length) return process.nextTick(function() {
    cb(null, null);
  });
  console.log(m.length);
  var response = {
    lastUsedAs: m && m.length ? m[0].type : null
  };
  // if the email is known, let's determine if the account has a password
  var m = jsel.match(":root > object:has(.emails > .?) > .password", [email], db.users);
  response.hasPassword = m.length ? (m[0].password && m[0].password.length) : false;
