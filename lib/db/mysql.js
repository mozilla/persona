...
exports.userOwnsEmail = function(uid, email, staged, cb) {
  ...
  client.query(
    ...
    function (err, rows) {
      if (err) return cb(err);
      var found = rows && rows.length === 1 && rows[0].n === 1;
      if (found || !staged) cb(err, found);
      else {
        client.query(
          'SELECT COUNT(*) AS n FROM stage WHERE existing_user = ? AND email = ?',
          [ uid, email ],
          function(err, rows) {
            found = rows && rows.length === 1 && rows[0].n === 1;
            cb(err, found);
          }
        );
      }
