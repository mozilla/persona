...
const schemas = [
  ...
  "CREATE TABLE IF NOT EXISTS email (" +
    ...
    // type is OBSOLETE.  can be dropped and removed once we fully migrate
    // to lastUsedAs.  For one train we will update type everywhere we
    // update lastUsedAs to preserve a rollback path
    "type ENUM('secondary', 'primary') DEFAULT 'secondary' NOT NULL," +
    "lastUsedAs ENUM('secondary', 'primary') DEFAULT 'secondary' NOT NULL," +
...
exports.emailInfo = function(email, cb) {
  client.query(
    "SELECT user.passwd AS hasPassword, " +
      "email.lastUsedAs as lastUsedAs FROM email, user " +
...
exports.emailType = function(email, cb) {
  client.query(
    "SELECT lastUsedAs FROM email WHERE address = ?", [ email ],
    function(err, rows) {
  ...
  client.query(
    ...
    function(err) {
      if (err) return cb(err);
        ...
        client.query(
          "INSERT INTO email(user, address, lastUsedAs, type) VALUES(?, ?, ?, ?)",
          [ userID, email, type, type ],
          function(err) {
            if (err) logUnexpectedError(err);
            cb(err, email, userID);
          });
...
exports.completePasswordReset = function(secret, cb) {
  getAndDeleteRowForSecret(secret, function(err, o) {
    ...
    exports.emailToUID(o.email, function(err, uid) {
      ...
      client.query(
        'UPDATE email SET verified = FALSE WHERE user = ? AND lastUsedAs = ? AND type = ? AND address != ?',
        [ uid, 'secondary', 'secondary', o.email ],
        function(err) {
          if (err) return cb(err);
...
exports.listEmails = function(uid, cb) {
  client.query(
    'SELECT address, lastUsedAs as type, verified FROM email WHERE user = ?',
      [ uid ],
      function (err, rows) {
        if (err) cb(err);
        else {
...
exports.updateEmailLastUsedAs = function(email, type, cb) {
  if (typeEnum.indexOf(type) === -1) {
    ...
    client.query('UPDATE email SET lastUsedAs = ?, type = ? WHERE address = ?', [type, type, email], cb);
  }
};
