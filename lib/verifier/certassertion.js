...
function verify(assertion, audience, forceIssuer, allowUnverified, successCB, errorCB) {
  ...
  jwcrypto.cert.verifyBundle(
    ...
    new Date(), function(issuer, next) {
      ...
      function success() {
        successCB(email, assertionParams.audience, assertionParams.expiresAt, ultimateIssuer, verified);
      }

      function mayNotSpeak() {
      ...
      if (ultimateIssuer !== domainFromEmail && ultimateIssuer !== forceIssuer) {
        // if we are the issuer, then dont check for delegation, cause
        // that means the domain isn't setup at all. However, we do need
        // to be sure it doesn't actually have primary support
        if (ultimateIssuer === HOSTNAME) {
          primary.currentSupport(domainFromEmail).then(function(support) {
            if (support.type === 'secondary') {
              success();
            } else {
              mayNotSpeak();
            }
