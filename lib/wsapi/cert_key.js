...
exports.process = function(req, res) {
  ...
  var forceIssuer = req.params.forceIssuer || 'default';

  var primarySupport;
  if (forceIssuer === 'default') {
    primarySupport = primary.currentSupport(primary.domainFromEmail(req.params.email));
  } else {
    primarySupport = Q.resolve();
  }

  var ownsEmail = Q.nfcall(db.userOwnsEmail, req.session.userid, req.params.email);
  var isVerified = Q.nfcall(db.emailIsVerified, req.params.email).fail(function(err) {
    if (err === 'no such email') {
      // no problem, clearly the answer is that the email is not verified
      return false;
    }
  ...
  Q.spread([primarySupport, ownsEmail, isVerified], function(support, owned, verified) {
    if (support.type === 'primary') return httputils.badRequest(res, "that email is backed by a primary");
  ...
  function forwardToKeysigner(unverified) {
    ...
    var body = {
      ...
      'forceIssuer': req.params.forceIssuer,
