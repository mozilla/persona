...
exports.process = function(req, res) {
  ...
  primary.verifyAssertion(req.params.assertion, function(err, email) {
    ...
    db.emailType(email, function(err, type) {
      ...
      if (! type) {
        // We haven't seen that email before, create a new account.
        return createAccountWithAssertion();
      }

      db.emailIsVerified(email, function(err, verified) {
        if (err) return wsapi.databaseDown(res, err);

        // If this is an unverified secondary email,
        // we've no reason to believe that the corresponding
        // uid actually owns that email.  Create a new account
        // in order to avoid email-squatting attacks such as
        // https://bugzilla.mozilla.org/show_bug.cgi?id=1301746
        if (type === 'secondary' && !verified) {
          return createAccountWithAssertion();
        }

        // Otherwise, authenticate as the existing uid.
      ...
      // Bounce off the dbwriter to have the user + email created
      function createAccountWithAssertion() {
        wsapi.requestToDBWriter({
