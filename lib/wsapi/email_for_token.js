...
exports.process = function(req, res) {
  ...
  db.emailForVerificationSecret(req.params.token, function(err, email, uid, passwd) {
    ...
    // If the user is on the same browser AND authenticated WITH A PASSWORD
    // they do not have to authenticate when doing one of the following:
    ...
    if ((req.params.token === req.session.pendingReverification ||
        ...
        req.session.auth_level === 'password')
    {
