...
// auth: <string> // authentication URL
// prov: <string> // certificate provisioning URL

// Firefox Desktop will use this wsapi for discovery (feature preffed off)
// it depends on auth and prov for loading the IdP (primary or fallback)
  ...
  if ('default' !== issuer) {
    ...
    primary.checkSupport(domain, function(err, r) {
      ...
      if (!err && r && r.urls) {
        ...
        typeRightNow = {
          auth: config.get('public_url') + '/auth#NATIVE',
          prov: config.get('public_url') + '/provision',
          type:'secondary',
          issuer: HOSTNAME,
