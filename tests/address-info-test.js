...
suite.addBatch({
  "creating lower case email (issues 2866 & 2891)": {
    topic: function() {
    ...
    "calling address_info with upper case user part": {
      topic: wsapi.get('/wsapi/address_info', {
        email: 'TEST@capitalize.example.com'
      }),
...
suite.addBatch({
  "creating camel cased email": {
    topic: function() {
      secondary.create({ email: "Test2@Capitalize.Example.Com" }, this.callback);
    },
    "works": function(e, r) {
      assert.isNull(e);
    },
    "calling address_info with upper case user part": {
      topic: wsapi.get('/wsapi/address_info', {
        email: 'TEST2@capitalize.example.com'
      }),
      "returns known": function(e, r) {
        assert.isNull(e)
        var r = JSON.parse(r.body);
